...
bool BGTactics::moveToStart(bool force)
{
    ...
    if (bgType == BATTLEGROUND_WS)
    {
        ...
        int startSpot = role < 4 ? BB_WSG_WAIT_SPOT_LEFT : role > 6 ? BB_WSG_WAIT_SPOT_RIGHT
                                                                    : BB_WSG_WAIT_SPOT_SPAWN;
...
bool BGTactics::atFlag(std::vector<BattleBotPath *> const &vPaths, std::vector<uint32> const &vFlagIds)
{
    ...
    for (ObjectGuid const guid : closeObjects)
    {
        ...
        bool atBase = bgType == BATTLEGROUND_WS ? go->GetEntry() == vFlagsWS[bot->GetTeamId()] : bgType == BATTLEGROUND_EY ? go->GetEntry() == vFlagsEY[0]
                                                                                                                           : false;
